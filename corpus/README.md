...
| `scores.json` | what every check put on every one of them | a run of `harvest.py` |
...
`families.py`, `planted.py` and `unread.py` read it from there, so run `decision.py` before them.
